...
- [ ] Real funding series per pair (funding_signal.py already fetches them)
      wired into apply_funding; re-run the r3 tables with real funding
- [ ] Maker-entry study: limit order at signal-bar close ± offset, honest
      fill model (filled only if next bar trades through the price); measures
      the fee 0.06%→−0.01% flip AND the missed-fill cost together
...
| 2026-07-05 | 4h + tp6 | 15.1% | 0.98 | −9.6% | G1–G4 ✓ | Phase-1 paper |
